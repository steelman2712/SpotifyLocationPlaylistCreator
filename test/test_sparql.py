from spotifylocation.sparql_query import (
    SparqlResultsFromArtist,
    SparqlResultsFromCoordinates,
)


def test_artist_query(mocker):
    expected_sparql_response = {
        "results": {
            "bindings": [
                {
                    "placeLabel": {"value": "TestPlace"},
    ...
    mocker.patch(
        "spotifylocation.sparql_query.SparqlResults.get_sparql_results",
        return_value=expected_sparql_response,
    )
